...
for data in tqdm(json_list, desc="processing json files"):
    for line in data:
        if "image" not in line:
            ...
            continue
        image_dir = line["image"].split("/")[0]
        first_conv = line["conversations"][0]
        first_conv["image"] = ["/".join(line["image"].split("/")[1:])]
        new_data.append({"id": line["id"], "image_dir": image_dir, "conversations": line["conversations"]})
print('keep data:', len(new_data))
sub_name = f"{len(new_data) // 1000}k"
output_path = input_path.stem + f"_{sub_name}.json"
