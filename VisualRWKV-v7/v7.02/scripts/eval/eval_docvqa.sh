...
output_file=$parent_dir/eval/docvqa/$SPLIT/$model_name/merge.jsonl
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat $parent_dir/eval/docvqa/$SPLIT/$model_name/${CHUNKS}_${IDX}.jsonl >> "$output_file"
done
