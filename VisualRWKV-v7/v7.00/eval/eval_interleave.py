...
if __name__ == "__main__":
    ...
    for pred in preds_all:
        dataset = pred["metadata"]['sub_task']
        if dataset not in preds_all_dict:
            preds_all_dict[dataset] = list()
        # add pred_response and gt_response and question_type and sample_id
        pred['sample_id'] = pred['question_id']
        pred['pred_response'] = pred['text']
        pred['gt_response'] = pred['metadata']['answer']
        pred['question_type'] = pred['metadata']['question_type']
        preds_all_dict[dataset].append(pred)
    ...
    with open(result_dir / 'eval_dataset.json', 'w') as f:
        json.dump(eval_result_list, f, indent=4)

    with open(result_dir / 'eval_dataset_details.json', 'w') as f:
    ...
    with open(result_dir /'eval_cat.json', 'w') as f:
