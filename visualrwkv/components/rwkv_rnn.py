...
class RWKV(MyModule):
    ...
    def forward(self, tokens=None, state=None, embs=None, full_output=False):
        with torch.no_grad():
            w = self.w
            args = self.args
            ...
            # tokens and embs are mutually exclusive
            if tokens is None and embs is None:
                raise ValueError('tokens and embs are both None')
            if tokens is not None and embs is not None:
                raise ValueError('tokens and embs are both not None')
            if tokens is not None:
                seq_mode = len(tokens) > 1
                x = w['emb.weight'][tokens if seq_mode else tokens[0]]
            else:
                seq_mode = len(embs) > 1
                x = embs if seq_mode else embs[0]
