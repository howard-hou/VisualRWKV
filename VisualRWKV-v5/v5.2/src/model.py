...
class TinyAttention(MyModule):
    ...
    def forward(self, x, x_emb):
        ...
        k = self.tiny_k(xx).view(B, T, self.n_head, self.head_size).transpose(1, 2) # (B, nh, L, hs)
        v = self.tiny_v(xx).view(B, T, self.n_head, self.head_size).transpose(1, 2) # (B, nh, L, hs)
