...
class VQAEval:
    ...
    def evaluate(self, quesIds=None):
        ...
        for quesId in quesIds:
            ...
            if avgGTAcc == 0:
                badcase_item = {
                    "question_id": quesId,
                    "question": res[quesId]["prompt"],
                    "gt_answer": gtAnswers,
                    "pred_answer": resAns,
                }
                self.setBadcase(quesId, badcase_item)
    ...
    def setBadcase(self, quesId, badcase):
        self.badcase[quesId] = badcase
...
if __name__ == "__main__":
    ...
    # loadRes has been modified to adapt to the visualrwkv format
    ...
    # output badcase
    for i, quesId in enumerate(vqa_scorer.badcase):
        print("Badcase %d:" % i, vqa_scorer.badcase[quesId])
        if i == 10:
            break
    # save metrics and badcase
    output_file = args.result_file.replace(".jsonl", "_metrics.jsonl")
    with open(output_file, "w") as f:
        f.write(json.dumps(metrics) + "\n")
        for quesId in vqa_scorer.badcase:
            f.write(json.dumps(vqa_scorer.badcase[quesId]) + "\n")